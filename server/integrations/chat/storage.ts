import { db } from "../../db";
import { conversations, messages, entities, relationsTable, recordings } from "@shared/schema";
import { eq, desc, and, or, sql, ilike } from "drizzle-orm";

export interface IChatStorage {
  getConversation(id: number): Promise<typeof conversations.$inferSelect | undefined>;
  getAllConversations(): Promise<(typeof conversations.$inferSelect)[]>;
  createConversation(title: string, recordingId?: number): Promise<typeof conversations.$inferSelect>;
...
export const chatStorage: IChatStorage = {
  ...
  async createConversation(title: string, recordingId?: number) {
    const [conversation] = await db.insert(conversations).values({ title, recordingId }).returning();
    return conversation;
  },
  ...
  async searchKnowledgeGraph(query: string, recordingId?: number) {
    ...
    if (allNodes.length === 0 && !recordingId) {
      console.log("[KG Retrieval] No entities found.");
      return { context: "", fallback: true };
    }

    // Fetch the transcript and summary for context enrichment
    ...
    return {
      context,
      fallback: !isStructural && !isAbstract && matchedEntities.length < 2 && !recordingId,
    };
  },
};
