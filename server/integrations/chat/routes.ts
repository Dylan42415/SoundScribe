...
export function registerChatRoutes(app: Express): void {
  // Get all conversations (optionally filtered by recordingId)
  app.get("/api/conversations", async (req: Request, res: Response) => {
    try {
      const recordingId = req.query.recordingId ? parseInt(req.query.recordingId as string) : undefined;
      const conversations = await chatStorage.getAllConversations();
      if (recordingId) {
        return res.json(conversations.filter(c => c.recordingId === recordingId));
      }
  ...
  app.post("/api/conversations", async (req: Request, res: Response) => {
    try {
      ...
      const conversation = await chatStorage.createConversation(title || "New Chat", recordingId);
